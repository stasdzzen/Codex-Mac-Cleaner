...
The owner authorization recorded above satisfies the legal gate. Do not start until CMC-11 and CMC-16 are merged and a separately launched Controller assigns CMC-01. Do not create an empty `NOTICE`, change product scope, merge, publish, or release.
...
Зафиксированное выше разрешение владельца закрывает legal gate. Не начинать до merge CMC-11 и CMC-16 и назначения CMC-01 отдельно запущенным Controller. Не создавать пустой `NOTICE`, не менять scope, не выполнять merge, publication или release.
