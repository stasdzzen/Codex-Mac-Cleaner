...
Replace the widget semantic theme tokens with the latest owner-supplied light and dark color, typography, radius and shadow values while keeping dark mode as the product default. Register each value in its correct Tailwind v4 theme namespace and keep the font stack local-only. Extend the semantic mapping for chart and sidebar tokens. Refine the existing Dashboard composition using installed radix-nova/shadcn components. Add an accessible relative comparison visualization for the existing server-owned byte metrics only, with decimal MB/GB labels and an explicit non-additive snapshot meaning. Improve active-audit progress motion and restrained content entry transitions with a `prefers-reduced-motion` fallback. Rebuild the autonomous Dashboard asset and update focused tests and requirements traceability.
