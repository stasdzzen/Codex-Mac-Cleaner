...
Главная поверхность v0.1 — правая панель Codex; fullscreen является обязательным адаптивным состоянием того же интерфейса. Язык интерфейса — русский. Тёмная тема включена по умолчанию.
...
- Основной узкий viewport: правая панель Codex около 520×900 px; Dashboard сохраняет компактную одноколоночную композицию.
- Desktop/fullscreen viewport: 1024–1440 px; контейнер ограничен примерно 1200 px, а независимые метрики используют сетку 8/4 вместо растягивания одной колонки.
- Side-panel и fullscreen — два состояния одного responsive React-интерфейса, а не две реализации.
