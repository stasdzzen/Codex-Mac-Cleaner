...
> [!WARNING]
> `v0.1.0-beta.6` — публичная тестовая версия. Автоматические проверки прошли
...
### Обновление с beta.5 до beta.6

Beta.5 уже содержит безопасный навык обновления. Удалять marketplace или
...
старый marketplace snapshot, затем повторно установите пакет из beta.6:
...
Beta.6 сохраняет отдельный безопасный навык обновления. После публикации
...
`v0.1.0-beta.6` добавляет пользовательские запросы fullscreen/PiP при
сохранении inline Dashboard по умолчанию и исправляет тайм-аут больших
аудитов. Package inventory теперь переиспользуется между кандидатами в
Snapshot A/B, а candidate correlation выполняется с ограниченной
параллельностью четыре и детерминированным порядком результатов. Совместимый
app-surface контракт Codex, безопасное обновление по точному тегу,
пятиминутный fail-closed deadline и security/privacy/contract gates сохранены.
Beta не является финальной v0.1: ручной Real-Mac smoke и итоговое решение
владельца остаются открытыми gates.
...
См. [заметки к v0.1.0-beta.6](docs/release/v0.1.0-beta.6.md).
