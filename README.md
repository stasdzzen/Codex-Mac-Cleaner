...
> [!WARNING]
> `v0.1.0-beta.8` — тестовая версия. Автоматические проверки проходят в
...
Codex Mac Cleaner проверяет пользовательскую папку Library и ищет следы
приложений, которых больше нет на Mac. Для каждого объекта он показывает:
...
Один раз добавьте репозиторий как источник плагина и установите текущую
версию:
...
## Как обновить с beta.7

Удалять marketplace или установленный плагин не нужно. Если у вас установлена
`v0.1.0-beta.7`, создайте новую задачу, выберите Codex Mac Cleaner и напишите:
...
APFS может совместно использовать блоки и обновлять показатели диска не сразу.
...
Первая версия работает только с пользовательской Library и остатками обычных
приложений. Системные операции в `/Library`, `sudo`, управление системными
службами, обновление Homebrew и глобальное управление средами разработки в неё
не входят.

Фоновая и автоматическая проверка пока не выполняется. Пользователь запускает
её сам кнопкой в интерфейсе.

Текущие заметки: [v0.1.0-beta.8](docs/release/v0.1.0-beta.8.md). Ручная
...
Перед изменениями прочитайте [CONTRIBUTING.md](CONTRIBUTING.md) и выберите одну
GitHub Issue. Об ошибке можно сообщить через Issue Form, а идеей поделиться в
