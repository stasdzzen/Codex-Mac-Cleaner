...
> [!WARNING]
> `v0.1.0-beta.4` — публичная тестовая версия для проверки на синтетических
...
старый marketplace snapshot, затем повторно установите пакет из beta.4:
...
### Обновления после beta.4 одной командой

Beta.4 содержит отдельный безопасный навык обновления. После публикации
следующего точного release-тега в новой задаче Codex достаточно написать,
например:
...
`v0.1.0-beta.4` включает безопасное обновление по точному тегу, живой Audit
Dashboard с пофазным прогрессом,
...
См. [заметки к v0.1.0-beta.4](docs/release/v0.1.0-beta.4.md).
