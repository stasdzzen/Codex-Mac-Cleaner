...
Issues созданы. Автоматические CMC-10 gates и verify-only package flow находятся
в репозитории; v0.1 не выпущена и всё ещё требует независимого review и ручных
owner gates.
...
## Release evidence без публикации
...
unsigned provenance. Verify-only режим не записывает artifact в репозиторий и
не создаёт tag, GitHub Release или публикацию.

Ручная проверка описана в
[незаполненном Real-Mac smoke protocol](docs/release/real-mac-smoke.md). Пока
владелец не выполнит его на review-approved SHA, этот gate остаётся открытым.
