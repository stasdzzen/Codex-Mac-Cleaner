...
function dashboardFromMeta(value: unknown): DashboardSnapshot | undefined {
  ...
  if (
    ...
    typeof dashboard.findingSummary.supportLevelCounts.analysisOnly !==
      "number" ||
...
function mergeSafeResult(
  ...
  return {
    ...
    ...(typeof safe.state === "string"
      ? { state: safe.state as DashboardSnapshot["state"] }
      : {}),
    ...(isRecord(safe.storageSummary)
      ? {
          storageSummary:
            safe.storageSummary as unknown as DashboardSnapshot["storageSummary"],
        }
      : {}),
    ...(isRecord(safe.diskObservation)
      ? {
          diskObservation:
            safe.diskObservation as unknown as DashboardSnapshot["diskObservation"],
        }
      : {}),
        ...
        <main className="mx-auto flex min-h-screen w-full max-w-3xl items-center p-6">
          <section className="w-full rounded-xl border bg-card p-6 text-card-foreground shadow-sm">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Найденные объекты и доступные действия появятся после ответа
              локального плагина.
            </p>
          </section>
        </main>
