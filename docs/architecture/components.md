...
React/Vite widget использует shadcn/ui на примитивах Base UI, стиль `base-mira` и тёмную тему. Основная компоновка — Audit Dashboard:
...
GitHub и действиями «Поделиться идеей», «Разработчик» и «Поддержать проект».
Назначения принадлежат закрытому allowlist репозитория, GitHub Discussions и
`dzzen.com`. Переход выполняет host через `openExternal` только после клика;
отсутствие capability или отказ не меняют аудит. Footer не вызывает MCP tools,
не сохраняет state и не добавляет telemetry. CSP разрешает только redirect origins
`https://github.com` и `https://dzzen.com`; внешние connect/resource/frame domains
отсутствуют.

Карточка finding показывает `SafeCorrelationView` с server-owned `FindingFacts`: очищенное имя одного верхнеуровневого компонента, категорию, размеры, время наблюдения, агрегированное owner binding, artifact/owner existence, activity/open-file/startup/uninstaller/receipt/dependency states, requirement profile/applicability, safe coverage gaps, `staleDuringAudit`, sensitivity flags, risk, rule explanation, `ReclaimEstimate`, recommended removal method и blocking reasons. Имя компонента доступно только widget `_meta`, не содержит directory chain и не заменяет generic model-visible label. UI не достраивает evidence, не выбирает profile, не трактует `unknown`/`not_applicable` как `absent` и не называет estimate фактическим изменением диска.
...
Dashboard показывает server-owned значения под понятными подписями «Размер найденных файлов», «Занимают на диске», «Хранится в карантине», «Удалено из карантина» и «Свободно на диске» в десятичных МБ или ГБ с максимум двумя знаками после запятой. Значение меньше 0,01 МБ показывается как `< 0,01 МБ`, а не как ноль. UI не суммирует данные сам, показывает время наблюдения диска и не трактует его изменение как результат purge.
...
* активные процессы с доказанно отсутствующим абсолютным executable как отдельная read-only диагностика без PID, пути и terminate action;
* пользовательский автозапуск как targeted read-only evidence: LaunchAgent с доказанно отсутствующим абсолютным target получает `analysis_only`; доступные системные LaunchAgents/LaunchDaemons с таким target получают только `unsupported_manual`;
* package receipts;
...
Candidate adapters обходят только девять allowlisted roots пользовательской `Library`. Targeted inspection системных и shared-источников, включая daemon с target в helpers, при доступной безопасной capability создаёт только `unsupported_manual` и никогда не расширяет mutation scope. Отдельный helper без authoritative launch relation не классифицируется по имени. Active TCP listeners, Homebrew services, cron, StartupItems и system extensions допускаются только как capability gap либо `unsupported_manual`; управление ими отсутствует.

Adapter не объявляет `absent` по пустому массиву и не создаёт общую identity через display name, basename или path. Raw claims существуют только в server process и не входят в `Observation`, persisted report или MCP output. Глобальный package inventory снимается один раз для Snapshot A и один раз для Snapshot B и переиспользуется candidate-specific receipt lookup; `pkgutil --file-info` остаётся отдельным для каждого кандидата в обеих фазах.
