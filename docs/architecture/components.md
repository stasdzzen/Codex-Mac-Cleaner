...
может запросить у Codex режим `fullscreen` или `pip`; автоматическое переключение
запрещено, а отказ либо отсутствие host capability сохраняют текущий вид и не меняют
состояние аудита. Публичный MCP Apps контракт не гарантирует размещение в правой
панели, поэтому плагин не заявляет такой режим и не использует недокументированные
metadata для его имитации.
