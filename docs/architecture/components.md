...
Хранит immutable safe reports, append-only journal, operation manifests, versioned keyed exclusions, keyed historical owner bindings и schedule state без базы данных. Raw identity graph, paths, inventory и tokens в audit report не сохраняются. Historical binding сохраняет только versioned keyed digests и validation metadata.
