...
Карточка finding показывает `SafeCorrelationView` с server-owned `FindingFacts`: компонент, категорию, размеры, время наблюдения, installed/activity/open-file/startup/receipt/dependency states, safe coverage gaps, `staleDuringAudit`, sensitivity flags, risk, rule explanation, `ReclaimEstimate`, recommended removal method и blocking reasons. UI не достраивает evidence, не трактует `unknown` как `absent` и не называет estimate фактическим изменением диска.
...
Строит server-only `CorrelationSubject`/`CorrelationEdge` из ephemeral raw claims, применяет versioned rules для filesystem/bundle/package/signing/owner identity, обнаруживает `ambiguous | missing | mismatch` и выпускает `CoverageCertificate` только после полного same-snapshot query. Public salt или plain hash не используются из-за dictionary-attack риска.

Resolver формирует immutable `CorrelationRevision`, `SafeCorrelationView` и candidate-specific facts. `absent` возможен только при полном source coverage; permission/capability gap, partial inventory, parse loss и ambiguity дают `unknown`.
...
Вычисляет `allowedActions` независимо от classifier. Учитывает `supportLevel`, категорию данных, protected scope, owner identity, installed state, dependencies, активность, открытые файлы, receipts, sensitivity flags, completeness certificates, correlation revision и Snapshot A/B fingerprints. Любые mandatory `unknown`, ambiguity, mismatch или stale state работают fail closed.
...
Хранит immutable safe reports, append-only journal, operation manifests, versioned keyed exclusions и schedule state без базы данных. Raw identity graph, paths, inventory и tokens в audit report не сохраняются.
