...
[ADR-0014](ADR-0014-defer-host-automation-post-v01.md) заменяет пункты 7–8 этого решения и производные release claims scheduling: host-native automation lifecycle и scheduled prompt перенесены после v0.1. `ScheduleIntent`/`ScheduleState`, tool skeleton и вкладка «Расписание» остаются в v0.1 только инертной compatibility groundwork с честным disabled/manual-run fallback. Запрет cron, LaunchAgent и скрытого scheduler не изменён.
...
* Persistent exclusions реализует `CMC-12`; post-v0.1 scheduling bridge остаётся в `CMC-13`, заблокированной до отдельного owner decision по ADR-0014.
