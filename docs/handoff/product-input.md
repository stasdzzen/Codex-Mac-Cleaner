...
Product-чат не добавляет bulk cleanup, автоматический purge, developer artifacts, cloud backend, analytics, privileged helper, Swift, SQLite, поддержку Intel или старых macOS. Такая потребность оформляется как отдельное предложение для Архитектора, а не как скрытая подзадача.
