...
3. Dashboard показывает прогресс, покрытие и таблицу находок.
...
Тёмный React/Vite widget на shadcn/ui показывает сводку, прогресс, таблицу, evidence Sheet, coverage Alert и отдельные AlertDialog для действий. Используются semantic tokens и автономный bundle без CDN.
...
* model-visible read tools и app-only mutation tools;
...
Лицензия, workspace, platform guard, schemas, storage и read-only MCP skeleton.
...
В релиз входят только `application_remnants`, read-only аудит, evidence, поэлементный quarantine/restore/purge и тёмный Dashboard.
