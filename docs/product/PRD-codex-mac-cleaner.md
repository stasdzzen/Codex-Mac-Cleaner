...
В релиз входят только `application_remnants`, read-only аудит с отменой, универсальные protected scopes, безопасные метаданные, evidence, inspect-only объяснения для всех Library categories и actionable `private_regenerable_remnant_v1` только для приватных cache/log, `unsupported_manual`, поэлементный quarantine/restore/purge, persistent exclusions, capability-aware ежемесячный read-only schedule либо честный fallback, Quarantine Center, честные метрики размера и тёмный Dashboard без терминального workflow.
