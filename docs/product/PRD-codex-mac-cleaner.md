...
Тёмный React/Vite widget на shadcn/ui открывается сразу после `audit_start` и показывает пять вкладок — «Обзор», «Находки», «Карантин», «Исключения», «Расписание» — пять server-owned показателей, живую фазу и candidate counts, таблицу, `supportLevel`, safe evidence `Sheet`, coverage `Alert` и отдельные `AlertDialog` для действий. До завершения `revision=null`, findings/actions пусты. Widget получает только `SafeCorrelationView` и opaque action handles, не получает raw path/inventory/identity/graph/token и не вычисляет completeness или policy. Используются semantic tokens, `Tabs`, `Button`, `Tooltip` и автономный bundle без CDN. `unsupported_manual` содержит объяснение «Требует расширенного режима» без shell-команды. Dashboard остаётся `inline` по умолчанию; кнопки пользователя могут запросить `fullscreen` или `pip`, если Codex поддерживает соответствующий режим. Отказ хоста не прерывает аудит. Гарантированное размещение в правой панели не входит в публичный контракт и не заявляется продуктом.

Большие аудиты не сокращают candidate discovery ради скорости. Global inventories, включая package inventory, переиспользуются между кандидатами в пределах Snapshot A/B, а candidate correlation выполняется с bounded concurrency четыре и детерминированным порядком результата. Пятиминутный deadline и fail-closed `AUDIT_TIMEOUT` не меняются.
