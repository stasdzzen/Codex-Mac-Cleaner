...
«Исключить» создаёт локальный `UserExclusion` в user Application Support, вне репозитория и `~/.codex`. Схема версионируется, запись атомарна, права ограничены текущим пользователем. Match использует rule/type/owner/bundle-package/signing/normalized-target identity, а не только строковый путь. Совпавший объект фильтруется до дорогого анализа и не получает destructive action token. Вкладка «Исключения» позволяет вернуть один объект в проверку и сбросить все правила только после подтверждения.
