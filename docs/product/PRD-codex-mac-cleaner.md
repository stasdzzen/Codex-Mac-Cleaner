...
6. Явная отмена read-only аудита оставляет безопасное terminal-состояние и прогресс,
   но не публикует частичную actionable revision и не разрешает mutation.
...
13. Последняя завершённая ревизия аудита переживает новую задачу Codex и открывается без автоматического повторного сканирования; любое файловое действие после restart проходит новую server-side revalidation.
...
3. Сразу после запуска открывается Dashboard v4 и показывает живую фазу, честные candidate counts, покрытие, свободное место в шапке и вкладки «Обзор», «Карантин», «Оставленные», «Автопроверка».
...
Во время долгого аудита пользователь может выбрать «Отменить аудит». Dashboard
показывает terminal-состояние и достигнутый progress, но частичная actionable
revision не публикуется; любое mutation-действие требует нового полностью
завершённого аудита.
...
Вкладка «Карантин» показывает записи, физический размер, время, статус и причину блокировки. «Очистить карантин» последовательно запрашивает отдельное подтверждение и выполняет purge для каждого payload; единого bulk token нет, а частичная ошибка оставляет остаток видимым. Сводка разделяет `candidateLogicalBytes`, `candidatePhysicalBytes`, `quarantinePhysicalBytes` и `purgedPhysicalBytes`. `DiskObservation` компактно показывает доступный и общий объём в шапке; ни одно поле не обещает точное изменение свободного места APFS. Все byte-значения UI представлены только в десятичных МБ/ГБ, включая явный `< 0,01 МБ` для малых ненулевых значений.

### Audit Dashboard

Тёмный React/Vite widget на shadcn/ui открывается сразу после `audit_start` и показывает четыре вкладки — «Обзор», «Карантин», «Оставленные», «Автопроверка». После завершения «Обзор» содержит ограниченную по высоте grouped-таблицу: кеши объединены как тип мусора, остатки удалённых приложений — по приложению, а действия раскрываются до одного finding. До завершения `revision=null`, findings/actions и отдельный storage-блок скрыты. Widget получает только `SafeCorrelationView` и opaque action handles, не получает raw path/inventory/identity/graph/token и не вычисляет completeness или policy. Используются semantic tokens, `Tabs`, `Button`, `Tooltip` и автономный bundle без CDN. `unsupported_manual` содержит объяснение «Требует расширенного режима» без shell-команды. Dashboard остаётся `inline` по умолчанию; кнопка «Развернуть» запрашивает `fullscreen`, а в развёрнутом режиме меняется на «Свернуть». Завершение аудита не меняет режим. `pip` отсутствует, отказ хоста не прерывает аудит.
...
Большие аудиты не сокращают candidate discovery ради скорости и не имеют общего
автоматического лимита времени. Global inventories, включая package inventory,
переиспользуются между кандидатами в пределах Snapshot A/B, а candidate
correlation выполняется с bounded concurrency восемь и детерминированным порядком
результата. Каждый найденный кандидат обрабатывается до публикации immutable
revision. Защитный таймаут отдельной системной команды становится fail-closed
coverage gap, но не отменяет весь аудит. Пользователь может остановить run только
явным `audit_cancel`.
