...
| `CMC-03` | Доменные schemas, JSON/NDJSON store и read-only MCP skeleton | `CMC-02` | Средний | Последовательно |
| `CMC-04` | Source adapters и capability report | `CMC-03` | Средний | Можно выполнять независимо от UI |
...
| `CMC-07` | Restore и ручной purge | `CMC-06` | Высокий: filesystem | Только последовательно |
| `CMC-08` | Тёмный Audit Dashboard на shadcn/ui с fixtures | `CMC-05` | Средний | Параллельно с `CMC-06` |
| `CMC-09` | Plugin manifest, Skill и полная MCP App integration | `CMC-07`, `CMC-08` | Средний | Последовательно |
| `CMC-10` | Security/privacy suite, clean-room packaging и release evidence | `CMC-09` | Высокий: release | Только последовательно; release не выполняется без владельца |
