...
Прочитай AGENTS.md, docs/architecture/runtime-flows.md, docs/contracts/domain-model.md, docs/contracts/quarantine-manifest.md, docs/decisions/ADR-0009-v01-safety-ux-completion.md, docs/decisions/ADR-0010-field-research-safety-contract.md, docs/safety/safety-model.md, docs/safety/path-policy.md, docs/quality/acceptance-gates.md и раздел CMC-07 плана. Dependency CMC-06 должна быть закрыта; Issue должна пройти validator.

Restore возвращает payload только в свободный исходный путь при существующем неизменном parent fingerprint. Нельзя создавать parent, перезаписывать объект или выбирать alternate destination. Используй same-volume atomic rename и проверяй metadata/xattrs. Move, restore и purge возвращают новый `stateVersion`, server-owned `StorageSummary` с logical/physical candidate bytes и timestamped `DiskObservation`.

Purge только ручной, по одному manifest/payload, после отдельного preview token. Он работает строго внутри quarantine root, использует lstat traversal и не следует symlinks. Успешный purge увеличивает вычисленный по локальному журналу `purgedPhysicalBytes`; `DiskObservation` — отдельное наблюдение `statfs`, причинное delta-поле запрещено. При ошибке запись карантина и сводка остаются неизменными. Автоматический TTL, bulk purge и «очистить всё» запрещены.
