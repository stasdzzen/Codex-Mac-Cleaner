...
Регрессии beta.11 проверяются отдельной матрицей:

* при отсутствии native tools Skill выполняет ровно один штатный host discovery, затем останавливается fail closed без Terminal, direct stdio, local HTML и auto-rescan;
* `audit_status.revision` равна `null` до успешного terminal state, а `completed | completed_with_warnings` возвращает точную integer revision для той же цепочки `audit_results`/`dashboard_open`;
* потеря in-memory run или cross-channel/stale cursor возвращает `AUDIT_STALE` без скрытого повторного аудита;
* выборки на 101 и 2 767 findings проходят все страницы без пропусков, дублей и массива больше 100 findings/512 КиБ;
* host notification той же revision не удаляет уже загруженные Widget-страницы и не уменьшает server-owned total.
...
* clean-room запуск в новой задаче Codex без копирования команд и с точной поверхностью из packaged probe;
