...
`~/APPS` и `~/.codex` не являются candidate roots и дополнительно защищены неизменяемыми prefix rules. Scanner не перечисляет их содержимое и не создаёт из них findings.
...
Targeted inspection может читать безопасные метаданные user/system LaunchAgents, `/Library/LaunchAgents`, `/Library/LaunchDaemons`, `/Library/PrivilegedHelperTools` и package receipts. Эти пути не добавляются в allowlist; их findings получают только `unsupported_manual`.

# Неизменяемые protected scopes

Server-side registry блокирует кандидаты или mutation для:

* canonical prefixes `~/APPS` и `~/.codex`, включая skills, MCP configs, plugins, sessions, history и settings;
* owner identities Telegram, Nicegram, Postman, pgAdmin, GitHub Desktop, Opera, Firefox, Slack, GRID, Disco Elysium, Microsoft Fonts и используемой Python.org 3.12;
* любого локального Git-проекта, если `.git` как файл или каталог найден в ancestry либо внутри объекта верхнего уровня.

Список встроен в policy package. UI preferences и MCP inputs не могут удалить правило. Имена выше задают product identity; реализация не угадывает bundle ID по строке и разрешает реальные IDs только по локальным installed-app metadata.
...
7. Проверить Protected Scope Registry, локальный Git marker, `supportLevel` и sensitivity flags.
8. Сверить fingerprint объекта и родительского каталога с ревизией или manifest.
9. Проверить отсутствие открытого файла и активного процесса, если правило этого требует.
10. Вычислить фиксированный destination `quarantine/<operation-id>/payload/object`.
11. Повторить критические проверки непосредственно перед atomic rename.
