...
* `Application Support`, `Containers`, `Preferences`, `WebKit`, `HTTPStorages` и `Saved Application State` доступны только для анализа;
* только отдельный artifact категории `cache | log`, прошедший `private_regenerable_remnant_v1`, может получить `prepare_move`;
...
9. Раздельно проверить `artifactExistenceState` cleanup-target и authoritative owner binding; legacy `targetExecutableState` не использовать.
10. Проверить server-owned `requirementProfileId=private_regenerable_remnant_v1`, category `cache | log` и applicability всех requirements.
11. Сверить immutable audit/correlation revision, Snapshot B fingerprints объекта/родителя и binding/profile/edge/coverage digests.
12. Повторить обязательные owner app/executable, process/open-file/startup/uninstaller/receipt/dependency queries; `absent` принять только с полным same-snapshot coverage, иначе блокировать.
13. Убедиться, что path/name/bundle-ID-only claim или user attestation не участвовали в owner binding и correlation graph не ambiguous/missing/mismatch/stale.
14. Убедиться, что `not_applicable` не заменил query failure и не подавил positive evidence.
15. Вычислить фиксированный destination `quarantine/<operation-id>/payload/object`.
16. Повторить критические проверки непосредственно перед atomic rename.
...
В публичных MCP schemas отсутствуют `path`, `destination`, glob, shell command, owner identity, requirement profile/applicability и флаг обхода политики. Exclusion input не принимает path или identity fields. Widget hydration не получает path, inventory, bundle/package/signing claims, historical binding, correlation graph или token material. Schedule input не принимает raw RRULE, cron expression, LaunchAgent или arbitrary prompt. UI не может предложить восстановление в другое место или принудительное продолжение.
