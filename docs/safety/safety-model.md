...
Продукт работает локально, не использует сеть и телеметрию. Логи не содержат содержимое файлов, секреты, subscription URLs, сырые ключи/значения конфигурации, полный путь или необработанный вывод утилит. Model-visible ответы используют display name, категорию, `supportLevel`, размер, безопасные flags и обезличенное местоположение. Публичный bundle, fixtures, snapshots, docs и PR evidence не содержат username, реальные домашние пути, app inventory или персональные решения разработчика.
