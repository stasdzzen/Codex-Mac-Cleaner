...
description: Базовый пошаговый TDD-план v0.1; дополнения публичного продукта вынесены в связанный план CMC-12–CMC-15.
...
* Built-in protected scopes используют только universal system/credential/browser-profile/personal/project/plugin/Codex classes; `~/.codex` и локальные Git-проекты защищены, а персональные app/path rules отсутствуют.
...
**Предусловие:** merge `CMC-11`, прямое разрешение владельца на legal action и label `cto:ready`.
...
it("не перечисляет protected roots как кандидаты", async () => {
  ...
  expect(result.findings).not.toEqual(expect.arrayContaining([
    findingFor(syntheticCodexStateRoot),
    findingFor(syntheticGitProjectRoot)
  ]));
});
...
Library adapter обходит только девять allowlisted roots и исключает quarantine/state roots, developer artifacts, external/network volumes, `~/.codex`, current project root, universal protected classes и найденные локальные Git-проекты. Он не перечисляет protected roots даже в warning details. Targeted inspection читает только безопасные metadata user/system launch/background items, `/Library/LaunchAgents`, `/Library/LaunchDaemons`, `/Library/PrivilegedHelperTools`, receipts, protected container metadata и доступные inspection-only system sources; missing executable, stale receipt и official uninstaller становятся typed evidence. Результат system scope всегда `unsupported_manual` с `allowedActions=["inspect"]`. `Operation not permitted` становится `PERMISSION_DENIED` coverage gap без `sudo` или TCC bypass guidance.
...
it.each([
  "PROTECT_CODEX_HOME", "PROTECT_CURRENT_PROJECT", "PROTECT_CREDENTIAL_STORE", "PROTECT_BROWSER_PROFILE", "PROTECT_LOCAL_GIT_PROJECT"
...
Policy независимо проверяет `supportLevel`, protected scope, category, sensitivity flags, confidence, app presence, dependencies, activity, open files, capability prerequisites, stale flag, owner, type, device, mount ID, ancestry, links, official uninstaller и `UserExclusion`. Built-in registry содержит только universal protected classes из ADR-0011 и детектор локального Git-проекта. Персональные app names/paths отсутствуют; tests используют synthetic identities.
...
it("показывает пять вкладок и не предлагает bulk purge", () => {
  ...
  expect(screen.getByRole("tab", { name: "Исключения" })).toBeVisible();
...
it("пропускает одну находку без server tool call", async () => {
  render(<AuditDashboard snapshot={actionableFixture} bridge={fakeBridge} />);
  await user.click(screen.getByRole("button", { name: "Пропустить сейчас" }));
  expect(fakeBridge.callTool).not.toHaveBeenCalled();
  expect(fakeBridge.lastViewState.skippedFindingIds).toContain("finding-1");
});
...
export interface WidgetBridge {
  ...
  setViewState(state: {
    activeTab: "overview" | "findings" | "quarantine" | "exclusions" | "schedule";
    filter: string;
...
Ожидаемый результат: UI tests проходят, включая tabs/cancellation/skip/support-level/finding-facts/metrics/quarantine/keyboard/focus; `dist/dashboard-v1.html` и local assets создаются без внешних URL и shell-command copy.
...
it.each(["synthetic-credential-store", ".codex", "synthetic-current-project", "synthetic-git-project"])(
...
it("проходит новый Codex task без терминального шага", async () => {
  ...
  expect(transcript).toEqual(expect.arrayContaining([
    "plugin_installed", "audit_started", "dashboard_opened", "skip_clicked",
...
* Дополнительный план `2026-07-17-public-plugin-v01-additions.md` является обязательным продолжением для CMC-12–CMC-15.
