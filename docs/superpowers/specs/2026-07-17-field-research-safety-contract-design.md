...
| Требует уточнения | Жёсткие исключения, безопасное чтение JSON/YAML/plist, кнопка «Оставить», системные находки без готовых команд, запуск без терминала | Добавить в канон, Product-документы и существующие Issues |
...
1. `~/APPS` и `~/.codex` не перечисляются как кандидаты, не обходятся рекурсивно и недоступны для mutation даже при подделанном запросе.
2. Внутри `~/.codex` отдельно считаются защищёнными skills, MCP-конфигурации, plugins, активные sessions, history и settings.
3. Защищены данные Telegram, Nicegram, Postman, pgAdmin, GitHub Desktop, профили Opera, Firefox и Slack, сохранения GRID и Disco Elysium, Microsoft Fonts и используемая установка Python.org 3.12.
...
5. Имена продуктов в каноне — product identities, а не предположение о bundle ID. Реальные идентификаторы сервер разрешает по локальным installed-app metadata; fixtures используют только синтетические продукты и bundle IDs.
...
# Пользовательский выбор «Оставить»

Кнопка «Оставить» — session-local no-op для текущей ревизии. Она:

* не вызывает MCP tool и не меняет файловую систему;
* добавляет `findingId` в `reviewedFindingIds` состояния представления widget;
* не создаёт постоянное исключение и не переносится в следующий аудит;
* не влияет на `allowedActions` и server policy.

Для actionable находки основные кнопки — «Оставить» и «Переместить в карантин». Окончательное удаление остаётся отдельным действием только во вкладке «Карантин» после нового preview.
...
4. Все решения принимаются кнопками «Оставить», «Переместить в карантин», «Восстановить» и «Удалить навсегда».
...
* browser profile, почту/мессенджер, game save, database, local Git project, template/settings и VPN subscription;
...
| `CMC-08` | «Оставить», `supportLevel`, `unsupported_manual`, пять независимых показателей и отсутствие shell UX |
