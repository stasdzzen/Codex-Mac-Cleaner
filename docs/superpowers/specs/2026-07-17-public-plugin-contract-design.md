...
| Ежемесячная проверка | Нет; фоновой очистки и LaunchAgent нет | Нет capability-aware Codex automation flow | Новая `CMC-13` |
...
* «Удалить» — пользовательское намерение безопасно убрать объект. В v0.1 оно всегда открывает отдельное подтверждение и затем перемещает один объект в собственный карантин; исходный объект не удаляется напрямую;
...
Подтверждение «Удалить» называет объект, оценку физического размера, риск, причину, способ восстановления и явно говорит, что объект будет перемещён в карантин. Один confirm относится только к одному объекту.
...
Расписание создаётся только после явного opt-in. Значение по умолчанию — выключено; рекомендуемая частота — раз в месяц. Пользователь видит день, локальное время, следующий и предыдущий запуск, но не raw RRULE.

MCP App не вызывает host-native automation напрямую. Поток:

1. Widget создаёт безопасный `request_schedule` intent с выбранными параметрами и показывает подтверждение.
2. Skill/host layer проверяет наличие поддержанной Codex automation capability.
3. При наличии capability host создаёт либо обновляет одну automation и возвращает opaque automation ID в local state.
4. Повторное включение обновляет существующую automation; pause, resume и delete используют тот же ID и не создают дубликат.
5. При отсутствии capability вкладка показывает disabled state и предлагает запускать аудит вручную; cron, LaunchAgent и скрытый scheduler не создаются.

Automation запускает только read-only `application_remnants`, применяет локальные исключения, ничего не удаляет, не запрашивает `sudo`, сообщает число findings и оценку размера, затем предлагает открыть интерактивный review.
...
Release-blocking tests покрывают Delete/Exclude/Skip-now, persistence и migration исключений, identity mismatch, невозможность mutation исключённого finding, official uninstaller preference, protected container metadata, stale receipt, active/open state, missing executable, отсутствие `sudo`/TCC bypass, scheduled read-only audit, dedup/update/pause/resume/delete automation, fallback без capability, redaction и отсутствие персональных данных разработчика в публичном bundle.
...
* `CMC-13` — capability-aware monthly audit schedule.
